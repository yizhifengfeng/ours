(function () {
    ...
    function render() {
        ...
        grid.querySelectorAll('.first-card-delete[data-id]').forEach(function (button) {
            button.addEventListener('click', function (event) {
                ...
                if (!recordId) return;
                window.FirstStore.deleteRecord(recordId).then(function () {
                    render();
    ...
    function handleSubmit(event) {
        ...
        window.FirstStore.addRecord({
            ...
            composeFeedback.hidden = false;
            composeFeedback.textContent = '保存失败：' + error.message;
        });
    }
    ...
    checkAdmin().then(function (ok) {
        ...
        return window.FirstStore.ensureLoaded();
    }).then(function () {
        render();
        if (shouldOpenComposer) openComposer();
    }).catch(function () {
        render();
    });
    bindComposerEvents();
