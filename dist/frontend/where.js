...
(function () {
    ...
    var adminCheckPromise = checkAdmin();
    adminCheckPromise.then(function () {
        return loadPlaces();
    }).then(function () {
        renderMarkers();
    }).catch(function () {
    ...
    if (currentCity && placeNameEl) {
        ...
        function createPhotoItem(photo) {
            ...
            div.innerHTML = '<a href="javascript:void(0)" class="photo-thumb"><img src="' + photo.imageUrl + '" alt="照片"></a>';
            if (isAdmin && photo.id) {
                var btn = document.createElement('button');
                btn.type = 'button';
                btn.className = 'photo-delete-btn';
                btn.setAttribute('data-photo-delete-id', photo.id);
                btn.setAttribute('aria-label', '删除照片');
                btn.textContent = '×';
                div.appendChild(btn);
            }
            return div;
        ...
        function appendPhotoItem(photo) {
            var node = createPhotoItem(photo);
            if (uploadWrap) {
                photoWall.insertBefore(node, uploadWrap.nextSibling);
            } else {
                photoWall.appendChild(node);
            }
            return node;
        }
        ...
        if (photoWall) {
            ...
            photoWall.addEventListener('click', function (e) {
                var deleteBtn = e.target && e.target.closest && e.target.closest('[data-photo-delete-id]');
                if (deleteBtn) {
                    if (!isAdmin) {
                        alert('仅管理员可删除地点照片。');
                        return;
                    }
                    var photoId = deleteBtn.getAttribute('data-photo-delete-id');
                    if (!photoId) return;
                    if (!window.confirm('确定删除这张照片吗？')) return;
                    deletePhoto(photoId).then(function () {
                        var item = deleteBtn.closest('.photo-item');
            ...
            if (photoUpload) {
                photoUpload.addEventListener('change', function () {
                    ...
                    for (var i = 0; i < files.length; i++) {
                        (function (file) {
                            readImageForStorage(file, function (dataUrl) {
                                addPhoto(currentCity, dataUrl).then(function (result) {
                                    appendPhotoItem({
                                        id: result && result.item ? result.item.id : '',
                                        imageUrl: dataUrl
                                    });
                                }).catch(function () {
                                    alert('保存失败，请重试。');
